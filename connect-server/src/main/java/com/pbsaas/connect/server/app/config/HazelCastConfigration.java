package com.pbsaas.connect.server.app.config;
...
@Configuration
@EnableCaching
@AutoConfigureAfter(value = {MarsConfig.class})
public class HazelCastConfigration {
    ...
    @Bean
    public HazelcastInstance hazelcastInstance(MarsConfig serverConfig) {
        log.debug("Configuring Hazelcast");
        HazelcastInstance hazelCastInstance =
                Hazelcast.getHazelcastInstanceByName("connect-server");
        if (hazelCastInstance != null) {
            log.debug("Hazelcast already initialized");
            return hazelCastInstance;
        }
        Config config = new Config();
        config.setInstanceName("connect-server");
        ...
        config.getMapConfigs().put("com.pbsaas.connect.domain.*",
    ...
    private MapConfig initializeDomainMapConfig(MarsConfig serverConfig) {
        MapConfig mapConfig = new MapConfig();
        mapConfig.setTimeToLiveSeconds(serverConfig.getHazelcast().getTimeToLiveSeconds());
        return mapConfig;
    }
